...
const UploadFeature = () => {
  ...
  const [uploading, setUploading] = useState(false);
    ...
    const handleBeforeUpload = (file: UploadFile, setFileList: React.Dispatch<React.SetStateAction<UploadFile[]>>) => {
        setFileList(prevFileList => [...prevFileList, file]);
        return false; // Prevent default upload behavior
      };
    
      const uploadFile = async (file: UploadFile, studentId: string | undefined) => {
        if (!studentId) {
      ...
      const uploadProps = (setFileList: React.Dispatch<React.SetStateAction<UploadFile[]>>): UploadProps => ({
        beforeUpload: (file) => handleBeforeUpload(file, setFileList),
        onRemove: (file) => {
          setFileList(prevFileList => prevFileList.filter(item => item.uid !== file.uid));
        },
        fileList: setFileList === setFileList1 ? fileList1 : 
                  setFileList === setFileList2 ? fileList2 : 
                  setFileList === setFileList3 ? fileList3 : 
                  fileList4
      });
    ...
    return (
      ...
      <Upload {...uploadProps(setFileList4)}>
        ...
        <Button
          ...
          disabled={fileList1.length === 0 && fileList2.length === 0 && fileList3.length === 0 && fileList4.length === 0}
          style={{ marginTop: 16 }}
        >
          Upload
        </Button>
